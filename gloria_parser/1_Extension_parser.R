...
for(year in years)
{
  ...
  # Compile GWP100 vector (kilo tons CO2eq)#this dimension is not available in my table
  #tmp_CO2 <- colSums(Q * unique$extension$CO2_eq)
  
  # Read material indices and aggregate to material groups (kilo tons)
  #i <- unique$extension$Lfd_Nr[ !is.na(unique$extension$materials) ]#this dimension is not available in my table
