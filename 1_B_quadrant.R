...
hybridise <- function(year, Sup, Use, Cou, Y_all) {
  ...
  if(year<1995){
    load(paste0("exiobase/pxp/1995_Z.RData"))
  } else {
    load(paste0("exiobase/pxp/", year, "_Z.RData"))
  }
  
  # Calculate Tech matrices for the 49 EXIO countries
  Tec <- vector("list", 49)
  for(i in 1:49) {
    tmp <- Matrix(0, nrow = 200, ncol = 200)#200 EXIOBASE products
    for(j in 1:49)
...
Y_all <- readRDS(paste0("fabio/Y.rds"))
...
for(i in seq_along(output)) {
  saveRDS(output[[i]], 
          paste0("fabio/hybrid/", years[[i]], "_B.rds"))
}
