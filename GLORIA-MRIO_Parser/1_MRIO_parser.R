...
for(year in years)
{
  ...
  fwrite( as.matrix(q), str_c(path$storeMRIOModel,year,"_x.csv") )#q is the x in EXIOBASE, the production
  fwrite( D, str_c(path$storeMRIOModel,year,"_D.csv") )#q is the x in EXIOBASE, the production
  fwrite( B, str_c(path$storeMRIOModel,year,"_B.csv") )
  
  #A and Z are also calculated in MATLAB, in MATLAB, it only takes a few minutes!
  ...
  #Z[is.na(Z)] <- 0                
  #Z[Z == Inf] <- 0
  #fwrite( Z, str_c(path$storeMRIOModel,year,"_Z.csv") )
  
  rm(T,S,U,B,D,Y_raw); gc()
  ...
  # Create inverse
  #R is too slow, it takes ~6 hours to get the leontief inverse, MATLAB is much more efficient, only severl minutes!
  #use MATLAB instead!
  #tryCatch({
  #  I <- diag( rep( 1,nrow(A) ) )
  #  L <- solve(I - A)#computationally singular
  #  L[is.na(L)] <- 0                
  #  L[L == Inf] <- 0
  #  
